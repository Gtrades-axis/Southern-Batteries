...
const wa = name =>
  `https://wa.me/254112323825?text=${encodeURIComponent(
    `Hello Southern Batteries, I am interested in ${name}. Please confirm availability and delivery.`
  )}`;
  ...
  return `
    <article class="card" data-category="${cat}">
      ...
      <div class="body">
        ...
        <a class="order" target="_blank" rel="noopener" href="${wa(p.name)}">Order / Enquire</a>
      </div>
    </article>`;
}
...
function render() {
  ...
  catalog.innerHTML = filtered.length
    ...
    : `<div class="empty">No batteries are listed in this category yet.</div>`;
...
onSnapshot(
  ...
  err => {
    console.error("Catalogue listener failed:", err);
    status.textContent = "Catalogue temporarily unavailable";
    catalog.innerHTML = `
      <div class="empty error">
        We are updating the catalogue. Please contact Southern Batteries on WhatsApp or phone.
      </div>`;
  }
);
